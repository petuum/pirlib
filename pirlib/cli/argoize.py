...
def config_argoize_parser(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("package", type=pathlib.Path)
    parser.add_argument("--target", type=str, required=True)
    parser.add_argument("-o", "--output", type=pathlib.Path)
    parser.set_defaults(parser=parser, handler=_argoize_handler)


def _argoize_handler(parser: argparse.ArgumentParser, args: argparse.Namespace) -> None:
    ...
    backend.generate_argo(package, args=args)
